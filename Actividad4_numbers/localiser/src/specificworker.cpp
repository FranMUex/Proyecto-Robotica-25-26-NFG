...
SpecificWorker::RetVal SpecificWorker::turn_to_color(const Corners &corners)
{
	...
	MODMNIST::MNISTResult result = mnist_proxy->getNumber();
	qInfo() << "Número: " << result.label;


	if (result.label > 0 && result.label <= 2)
    {
        habitacion = result.label - 1;
    	...
    	current_door = (door_crossing.entering_door_index + 1) % nominal_rooms[habitacion].doors.size();
...
SpecificWorker::RetVal SpecificWorker::cross_door(const RoboCompLidar3D::TPoints &points)
{
	...
	else
	{
		...
		if (std::chrono::duration_cast<std::chrono::milliseconds>(elapsed).count() > 2800)
		{
			first_time = true;
